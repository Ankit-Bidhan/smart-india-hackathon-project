    ...
    useEffect(() => {
        const loadApprovedGems = async () => {
            try {
                ...
                console.error("Unable to load hidden gems:", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <section className="hidden-gems-section" id="hidden-gems">
            <div className="hidden-gems-header">
                <div>
                    <p className="section-label">OFF THE BEATEN PATH</p>
            ...
            ) : approvedCommunityGems.length === 0 ? (
                <p>
                    No approved hidden gems yet. Local contributors can add
                    new discoveries from their profile.
                </p>
            ) : (
                <div className="hidden-gems-grid">
                    {approvedCommunityGems.map((place) => (
