...
function cleanAIText(text) {
    return text.replace(/^#{1,6}\s*/gm, "").replace(/\*\*(.*?)\*\*/g, "$1").replace(/\*(.*?)\*/g, "$1").replace(/^-{3,}$/gm, "").replace(/^\s*[-*]\s+/gm, "• ").replace(/\n{3,}/g, "\n\n").trim();
}

function AIGuide() {
    const [messages, setMessages] = useState([
        { role: "assistant", text: "Hi! I'm your AI travel guide. 👋" },
        { role: "assistant", text: "Tell me where you want to go and I'll help you plan your journey." },
    ]);
    ...
    const listenToMessage = async (text, index) => {
        ...
        try {
            const response = await fetch("/.netlify/functions/tts", {
                ...
                body: JSON.stringify({ text: cleanAIText(text) }),
            });

            if (!response.ok) {
                // Local/development fallback if ElevenLabs is not configured.
                if ("speechSynthesis" in window) {
                    const utterance = new SpeechSynthesisUtterance(cleanAIText(text));
    ...
    const sendMessage = async (messageText = input) => {
        ...
        try {
            ...
            setMessages((previous) => [...previous, { role: "assistant", text: "Sorry, I couldn't connect to the AI right now. Please try again." }]);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <section className="ai-section" id="ai-guide">
            <div className="ai-heading">
                ...
                <p>Get personalized recommendations, trip ideas, local insights and answers to your travel questions.</p>
            </div>
            <div className="ai-container">
                <div className="ai-top"><div className="ai-avatar">🤖</div><div><h3>TravelEase AI</h3><span>● Online</span></div></div>
                <div className="chat-area">
                    {messages.map((message, index) => (
                        <div key={index} className={message.role === "assistant" ? "ai-message" : "user-message"}>
                            {message.role === "assistant" && <span>🤖</span>}
                            <div className="ai-message-content">
                                <div>{message.role === "assistant" ? cleanAIText(message.text) : message.text}</div>
                                {message.role === "assistant" && <button type="button" className="ai-listen-btn" onClick={() => listenToMessage(message.text, index)}>{speakingIndex === index ? "⏹ Stop" : "🔊 Listen"}</button>}
                            </div>
                        </div>
                    ))}
                    {loading && <div className="ai-message"><span>🤖</span><div className="typing">Thinking... 💭</div></div>}
                </div>
                ...
                <form className="ai-input" onSubmit={handleSubmit}>
                    <input type="text" value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask anything about your trip..." disabled={loading} />
                    <button type="submit" disabled={loading || !input.trim()}>{loading ? "..." : "➤"}</button>
                </form>
            </div>
        </section>
    );
