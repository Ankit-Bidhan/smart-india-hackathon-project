...
function NearbyPlaces() {
    ...
    const findNearbyPlaces = () => {
        ...
        navigator.geolocation.getCurrentPosition(async ({ coords }) => {
            try {
                ...
                const results = data.elements.map((element) => {
                    ...
                    return {
                        ...
                        emoji: emoji(element.tags),
                        distance: distanceKm(latitude, longitude, p.lat, p.lon),
                        lat: p.lat,
                        lon: p.lon,
                    };
            ...
            setLoading(false);
            setError(geoError.code === geoError.PERMISSION_DENIED ? "Location permission was denied. Allow location and try again." : "We could not get your location. Please try again.");
    ...
    return (
        <section className="nearby-places-section" id="nearby-places">
            <div className="nearby-header">
                <div>
                    <p className="section-label">DISCOVER AROUND YOU</p>
                    <h2>📍 Places near you</h2>
                    <p>Choose a radius and discover interesting places around your current location.</p>
                </div>
                <div className="nearby-controls">
                    ...
                    <button type="button" className="nearby-button" onClick={findNearbyPlaces} disabled={loading}>
                        {loading ? "Finding places..." : "📍 Get places nearby"}
                    </button>
                </div>
            </div>
                ...
                <div className="nearby-grid">
                    {places.map((place) => (
                        <article className="nearby-card" key={place.id}>
                            <div className="nearby-card-icon">{place.emoji}</div>
                            <div className="nearby-card-content">
                                <span className="nearby-category">{place.category}</span>
                                <h3>{place.name}</h3>
                                <p>📍 {place.distance < 1 ? `${Math.round(place.distance * 1000)} m` : `${place.distance.toFixed(1)} km`} away</p>
                                <a className="nearby-map-link" href={`https://www.openstreetmap.org/?mlat=${place.lat}&mlon=${place.lon}#map=17/${place.lat}/${place.lon}`} target="_blank" rel="noreferrer">View on map →</a>
                            </div>
                        </article>
                    ))}
                </div>
